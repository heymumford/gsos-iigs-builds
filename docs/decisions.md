...
## D-012 — Pi 4 sidecar bridge as a product axis (deferred — open dimensions documented)

**As of:** 2026-05-05

**Context.** A Cowork session surfaced a product axis adjacent to this repo's build-infrastructure scope: a Raspberry Pi 4 sidecar attached to the IIGS over the port the bridge design ultimately selects, providing modern Ethernet, WiFi, TLS, DNS, file-share, and protocol bridging — shipped as a hardware-plus-software kit. This axis differs from D-009 (clean-room deconstruction) and D-011 (unlock extensions): it ships physical hardware, raises distribution-model questions distinct from the patches-only convention here, and depends on a working IIGS-side bridge driver that itself fits the D-011 unlock-extensions pattern.

**Decision.** Deferred. The IIGS-side bridge driver fits within D-011 as a net-new system extension (under `*/SYSTEM/DRIVERS/`) and may be developed here when its preconditions are met. The product packaging — port choice, PCB design, kit assembly, distribution model, legal posture for shipping — is out of scope for this repository. When a real driver lands and a real bandwidth measurement exists, a follow-up decision (or a sibling repo) addresses the product axis with these open dimensions on the table:

- **Port:** SmartPort (Floppy Emu precedent, ~30-50 KB/s real, in-spec connector, easiest ship) vs. slot card (200-500 KB/s real, requires PCB, Uthernet II precedent) vs. SCSI bridge (sourcing-constrained on the IIGS-card side) vs. SCC serial (legacy LocalTalk path, too slow for modern web).
- **Marinetti boundary:** keep Marinetti as the IIGS-side TCP/IP API and dispatch to the Pi bridge from beneath it (preserves Spectrum / SAFE2 / Sweet16 / Casper compatibility) vs. fork.
- **Pi-side language:** Rust (smaller binary, slower iteration, easier cross-compile) vs. Python (faster iteration, larger ship surface, richer protocol library ecosystem).
- **Pi 4 specifically?** vs. Pi 5 (more power draw, hotter) vs. Pi Zero 2 W (fits inside the IIGS case but no built-in Ethernet).
- **Distribution model:** patches-only against customer-supplied baseline (legally safest, customer assembly burden) vs. Apple license request (precedent thin) vs. clean-room driver shipped alone with no GS/OS modifications (smallest blast radius, requires customer to install).

**Consequence.** No work in this repository on the product axis until two preconditions hold: (a) Phase 5 (emulator boot) and Phase 6 (Ghidra deconstruction) are sustained green, (b) a real bridge-driver prototype is running against an emulated SmartPort with measured bandwidth. Conversation about kit packaging, PCB fab, and shipping is documented here as "deferred until preconditions" rather than acted on. When preconditions hold, the open dimensions above become the agenda for D-013 or a sibling repo's first decision.
