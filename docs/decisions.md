...
- **Port:** SmartPort (FloppyEmu precedent, ~30-50 KB/s real, in-spec connector, easiest ship) vs. slot card (200-500 KB/s real, requires PCB, Uthernet III precedent) vs. SCSI bridge (sourcing-constrained on the IIGS-card side) vs. SCC serial (legacy LocalTalk path, too slow for modern web).
