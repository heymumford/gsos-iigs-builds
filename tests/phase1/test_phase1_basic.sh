#!/usr/bin/env bash
# Basic sanity test for phase1-hardware.sh
# Verifies:
...
script="$(dirname "$0")/../../phases/phase1-hardware.sh"

if [[ ! -x "$script" ]]; then
  echo "✗ FAIL: phase1-hardware.sh not executable"
  exit 1
fi
...
if ! echo "$output" | grep -q '\[phase1\]'; then
  echo "✗ FAIL: no [phase1] notes found"
  exit 1
fi

echo "✓ PASS: phase1-hardware.sh output is valid"
exit 0
